...
// Copies the member data
  void
  VanillaOption::copy (const VanillaOption& rhs)
  {
    K = rhs.getK ();
    r = rhs.getr ();
    T = rhs.getT ();
...
// the cumulative distribution function of the normal distribution)
  double
  VanillaOption::calc_call_price () const
  {
...
// the cumulative distribution function of the normal distribution)
  double
  VanillaOption::calc_put_price () const
  {
    double sigma_sqrt_T = sigma * sqrt (T);
    double d_1 = (log (S / K) + (r + sigma * sigma * 0.5) * T) / sigma_sqrt_T;
    double d_2 = d_1 - sigma_sqrt_T;
